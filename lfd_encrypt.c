...
#include <string.h>
#include <time.h>
#include <arpa/inet.h>	/* htonl/ntohl prototypes (sideband sequence in non-ECB modes) */
...
static int alloc_encrypt(struct vtun_host *host)
{
   ...
   phost = host;
   cipher = host->cipher;
   if (!ctx_enc) ctx_enc = EVP_CIPHER_CTX_new();
   if (!ctx_dec) ctx_dec = EVP_CIPHER_CTX_new();
   if (!ctx_enc_ecb) ctx_enc_ecb = EVP_CIPHER_CTX_new();
   if (!ctx_dec_ecb) ctx_dec_ecb = EVP_CIPHER_CTX_new();
   switch(cipher)
   {
   ...
   EVP_CIPHER_CTX_reset(pctx_enc);
   EVP_CIPHER_CTX_reset(pctx_dec);
...
static int free_encrypt()
{
   ...
   EVP_CIPHER_CTX_reset(ctx_enc);
   EVP_CIPHER_CTX_reset(ctx_dec);
   EVP_CIPHER_CTX_reset(ctx_enc_ecb);
   EVP_CIPHER_CTX_reset(ctx_dec_ecb);
