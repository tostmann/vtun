...
int main(int argc, char *argv[], char *env[])
{
      ...
      * provider, which is NOT loaded by default.  vtun's default wire cipher
      * blowfish128ecb ("encrypt yes") and lfd_legacy_encrypt need it; without it
      * EVP_EncryptInit_ex() on EVP_bf_ecb() fails, vtund ignores the error and
      * then dereferences the half-initialised context -> SIGSEGV in libcrypto.
      * Load legacy+default in-process so it works irrespective of OPENSSL_CONF
      * (which daemonize() strips from the environment). */
